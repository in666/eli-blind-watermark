package com.eli.watermark.controller;

import com.eli.Application;
import com.eli.watermark.utils.Base64Utils;
import com.eli.watermark.utils.QRCodeUtils;
import com.eli.watermark.utils.WaterMarkDCT;
import com.eli.watermark.utils.WaterMarkDFT;
    ...
    public void addWaterMark(MultipartFile image, boolean isText, String watermark, HttpServletResponse response) {
        try {
            ...
            if (checkImage(image, isText)) {
                LoggerFactory.getLogger(Application.class).info("图片格式不正确或不符合要求");
                return;
            }
            // 1、获取文件名后缀
            ...
            watermark = Base64Utils.textToBase64Str(watermark);
    ...
    public byte[] getWaterMark(MultipartFile picture, boolean isText) {
        try {
            ...
            if (isText) {
                ...
                System.out.println(Base64Utils.base64StrToText(QRCodeUtils.QRReader(outputPath)));
            }
            // 返回图片
